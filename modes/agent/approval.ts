...
/**
 * Bundles a flat list of pending actions into reviewable groups for approval.
 *
 * - Shell commands (`tool_execute`) become one group each, labeled by command.
 * - All other actions are grouped by their target file path.
 *   - For each path, actions are sorted chronologically. If every action is a
 *     `folder_create`, the group is labeled as a folder creation with no patch.
 *   - Otherwise the actions are folded into a single before/after state and
 *     rendered as a unified diff patch, labeled with the path and action kinds.
 *
 * Path groups come first (sorted alphabetically), then shell groups.
 */
